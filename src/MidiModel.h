...
public:
	...
	void registerEvent(int channel, int status, int byteOne, int byteTwo, midi_ct comparisonType, string helpMessage, string functionToCall) {
		...
		if (comparisonType == kMIDI_PASS_PARAM_ONE) {
			registerEvent(channel, status, byteOne, byteTwo, kMIDI_PASS_PARAM_ONE_INTERNAL_USE, helpMessage, functionToCall, -1);
			return;
		} else if (comparisonType == kMIDI_PASS_PARAM_TWO) {
			registerEvent(channel, status, byteOne, byteTwo, kMIDI_PASS_PARAM_TWO_INTERNAL_USE, helpMessage, functionToCall, -1);
			return;
		} else if (comparisonType == kMIDI_PASS_PARAM_BOTH) {
			registerEvent(channel, status, byteOne, byteTwo, kMIDI_PASS_PARAM_BOTH_INTERNAL_USE, helpMessage, functionToCall, -1, -1);
			return;
		}
	...
	void registerEvent(int channel, int status, int byteOne, int byteTwo, midi_ct comparisonType, string helpMessage, string functionToCall, Param1 param1) {
		...
		if (comparisonType == kMIDI_PASS_PARAM_ONE) {
			registerEvent(channel, status, byteOne, byteTwo, kMIDI_PASS_PARAM_ONE_INTERNAL_USE, helpMessage, functionToCall, param1, -1);
			return;
		} else if (comparisonType == kMIDI_PASS_PARAM_TWO) {
			registerEvent(channel, status, byteOne, byteTwo, kMIDI_PASS_PARAM_TWO_INTERNAL_USE, helpMessage, functionToCall, param1, -1);
			return;
		} else if (comparisonType == kMIDI_PASS_PARAM_BOTH) {
			LOG_ERROR("Maximum of two parameters, so can't pass both param1, byteOne AND bytTwo as well!!");
			abort();
		} else if (comparisonType == kMIDI_PASS_PARAM_ONE_INTERNAL_USE) {
			comparisonType = kMIDI_PASS_PARAM_ONE;
		} else if (comparisonType == kMIDI_PASS_PARAM_TWO_INTERNAL_USE) {
			comparisonType = kMIDI_PASS_PARAM_TWO;
		}
	...
	void registerEvent(int channel, int status, int byteOne, int byteTwo, midi_ct comparisonType, string helpMessage, string functionToCall, Param1 param1, Param2 param2) {
		...
		if (comparisonType == kMIDI_PASS_PARAM_ONE || comparisonType == kMIDI_PASS_PARAM_TWO || comparisonType == kMIDI_PASS_PARAM_BOTH) {
			...
			comparisonType = kMIDI_PASS_PARAM_TWO;
		} else if (comparisonType == kMIDI_PASS_PARAM_BOTH_INTERNAL_USE) {
			comparisonType = kMIDI_PASS_PARAM_BOTH;
		}
	...
	string getAllMidiUsageAsList(bool log = false) {
		...
		if (it_type != _mappedMidiType.end()) {
			switch (comparisonType) {
				case kMIDI_ANY:
					...
					break;
				case kMIDI_PASS_PARAM_ONE:
				case kMIDI_PASS_PARAM_TWO:
				case kMIDI_PASS_PARAM_BOTH:
					comparisonMatched = true;
					break;
				case kMIDI_EQUAL:
		...
		if (comparisonMatched) {
			...
			switch (dt) {
				...
				case GO_DELEGATE_INT:
				{
					...
					funcToExec.SetMemento(dm);
					if (comparisonType == kMIDI_PASS_PARAM_ONE) {
						funcToExec(midiMessage.getByteOne());
					} else if (comparisonType == kMIDI_PASS_PARAM_TWO) {
						funcToExec(midiMessage.getByteTwo());
				...
				case GO_DELEGATE_INT_INT:
				{
					...
					funcToExec.SetMemento(dm);
					if (comparisonType == kMIDI_PASS_PARAM_ONE) {
						funcToExec(it2ii->second.getInputParams().getParam1(), midiMessage.getByteOne());
					} else if (comparisonType == kMIDI_PASS_PARAM_TWO) {
						funcToExec(it2ii->second.getInputParams().getParam1(), midiMessage.getByteTwo());
					} else if (comparisonType == kMIDI_PASS_PARAM_BOTH) {
						funcToExec(midiMessage.getByteOne(), midiMessage.getByteTwo());
				...
				case GO_DELEGATE_STRING_INT:
				{
					...
					funcToExec.SetMemento(dm);
					if (comparisonType == kMIDI_PASS_PARAM_ONE) {
						funcToExec(it2si->second.getInputParams().getParam1(), midiMessage.getByteOne());
					} else if (comparisonType == kMIDI_PASS_PARAM_TWO) {
						funcToExec(it2si->second.getInputParams().getParam1(), midiMessage.getByteTwo());
				...
				case GO_DELEGATE_FLOAT_INT:
				{
					...
					funcToExec.SetMemento(dm);
					if (comparisonType == kMIDI_PASS_PARAM_ONE) {
						funcToExec(it2fi->second.getInputParams().getParam1(), midiMessage.getByteOne());
					} else if (comparisonType == kMIDI_PASS_PARAM_TWO) {
						funcToExec(it2fi->second.getInputParams().getParam1(), midiMessage.getByteTwo());
				...
				case GO_DELEGATE_BOOL_INT:
				{
					...
					funcToExec.SetMemento(dm);
					if (comparisonType == kMIDI_PASS_PARAM_ONE) {
						funcToExec(it2bi->second.getInputParams().getParam1(), midiMessage.getByteOne());
					} else if (comparisonType == kMIDI_PASS_PARAM_TWO) {
						funcToExec(it2bi->second.getInputParams().getParam1(), midiMessage.getByteTwo());
